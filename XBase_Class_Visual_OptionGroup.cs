...
namespace JAXBase
{
    public class XBase_Class_Visual_OptionGroup : XBase_Class_Visual
    {

        List<RadioButton> MyObjList = [];

        public Panel oGrp => (Panel)me.visualObject!;

        public XBase_Class_Visual_OptionGroup(JAXObjectWrapper jow, string name) : base(jow, name)
        {
            SetVisualObject(new Panel(), "OptionGroup", "optgroup", true, UserObject.URW);
        }
                ...
                else
                {
                    switch (propertyName)
                    {
                        ...
                        case "borderstyle":
                            ...
                            {
                                UserProperties[propertyName].Element.Value = objtk.AsInt();
                                oGrp.BorderStyle = objtk.AsInt() == 0 ? BorderStyle.None : BorderStyle.FixedSingle;
                            }
                            else
                                throw new Exception("11|");
                            break;
                            ...
                            if (objtk.Element.Type.Equals("N"))
                            {
                                ...
                                for (int i = 0; i < bc.Count; i++)
                                {
                                    if (bc._avalue[i].Value is JAXObjectWrapper)
                                    {
                                        ...
                                        if (btn.BaseClass.Equals("optionbutton", StringComparison.OrdinalIgnoreCase))
                                            currentButtonCount++;
                                    }
                                }
                                ...
                                while (ii >= 0 && currentButtonCount > desiredButtonCount)
                                {
                                    if (bc._avalue[ii].Value is JAXObjectWrapper)
                                    {
                                        ...
                                        {
                                            bc.RemoveAt(ii);
                                            oGrp.Controls.RemoveAt(oGrp.Controls.Count - 1);
                                            ii--;
                                            currentButtonCount--;
                                        }
                                    }
                                ...
                                while (currentButtonCount < desiredButtonCount)
                                {
                                    ...
                                    obut.SetProperty("caption", $"Option{bc.Count + 1}");
                                    obut.SetProperty("visible", true);
                                    obut.SetParent(me);
                                    bc.Add(obut);
                                    oGrp.Controls.Add(obut.visualObject);
                                    currentButtonCount++;
                                }
        ...
        public override string[] JAXProperties()
        {
            return
                [
                "anchor,n,0","autosize,l,false","backcolor,R,240|240|240","backstyle,n,1",
                "BaseClass,C,Grid","bordercolor,R,0|0|0","borderstyle,n,1","borderwidth,n,0",
                "buttoncount,n,0","buttonlayout,n,0",
                "Class,C,OptionGroup","ClassLibrary,C,","colorscheme,n,1","colorsource,n,4","Comment,C,","controlcount,n,0",
                "default,l,false","disabledbackcolor,R,128|128|128","disabledforecolor,R,64|64|64",
                ...
                "value,n,1","visible,l,true","width,N,94"
                ];
        }
                ...
                if (btncount > 0)
                {
                    ...
                    for (int i = 0; i < otk._avalue.Count; i++)
                    {
                        ...
                        if (otk._avalue[i].Value is JAXObjectWrapper)
                        {
                            ...
                            if (itk.BaseClass.Equals("optionbutton", StringComparison.OrdinalIgnoreCase))
                            {
                                if (UserProperties["buttonlayout"].AsInt() == 2)
                                {
                                    // Freeform layout
